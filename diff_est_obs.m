...
ax1_pos = [left_margin, btm_margin, map_w, map_h];               % 地图
ax2_pos = [left_margin+map_w+fig_margin_h+0.025, btm_margin+2*(right_h+fig_margin_v), right_w, right_h]; % 预测 δ13C
ax3_pos = [left_margin+map_w+fig_margin_h+0.025, btm_margin+right_h+fig_margin_v, right_w, right_h];     % 观测 δ13C
ax4_pos = [left_margin+map_w+fig_margin_h+0.025, btm_margin, right_w, right_h];                              % 预测-观测
...
c2 = colorbar(ax2);
pos = get(c2,'Position');
pos(1) = pos(1) + 0.07;   % 向右移动
pos(3) = pos(3) * 0.5;     % 缩窄
set(c2, 'Position', pos);
% set(c2, 'Position', get(c2,'Position') .* [1 1 0.6 1]);  % 宽度缩小一半
...
c3 = colorbar(ax3); 
% set(c3, 'Position', get(c3,'Position') .* [1 1 0.6 1]);  % 宽度缩小一半
pos = get(c3,'Position');
pos(1) = pos(1) + 0.07;   % 向右移动
pos(3) = pos(3) * 0.5;     % 缩窄
set(c3, 'Position', pos);
...
c4 = colorbar(ax4); 
% set(c4, 'Position', get(c4,'Position') .* [1 1 0.6 1]);  % 宽度缩小一半
pos = get(c4,'Position');
pos(1) = pos(1) + 0.07;   % 向右移动
pos(3) = pos(3) * 0.5;     % 缩窄
set(c4, 'Position', pos);
